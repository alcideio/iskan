...
func GenerateAdvisorReport(res *types.ScanTaskResult) (*AdvisorClusterReport, error) {
	...
	var missingImageInfo, failedImages, theContainers sets.String
	...
	for _, pod := range res.ScannedPods {
		...
		fmt.Fprintf(msgBuf, "Total number of identified container image vulnerabilties in '%v/%v' by severity: %v\n", pod.Namespace, pod.Name, podSummary.String())
		...
		check.Recommendation = `If vulnerabilities detected inside your container images, it is recommended to build images using "Distroless" images. "Distroless" images contain only your application and its runtime dependencies. 
...
It improves the signal to noise ratio generated by container image vulnerability scanners and reduces the burden of establishing provenance to just what you need.
`
		...
		case "":
			if failedImages.Len() > 0 {
				...
				if theContainers.Len() > 0 {
					fmt.Fprintf(msgBuf,
						"The following container images were scanned: '%v'\n",
						strings.Join(theContainers.List(), ","))
				}
			}
		}
