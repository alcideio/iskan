...
function AdditionalFindings({check}) {
    if (check.AdditionalFindings === null || check.AdditionalFindings === undefined) {
        return (<></>);
    }

    if (check.AdditionalFindings.iskan === null || check.AdditionalFindings.iskan === undefined) {
        return (<></>);
    }

    console.log("check.AdditionalFindings", check.AdditionalFindings.iskan);
    let findings = check.AdditionalFindings.iskan.Findings
    ...
    return <CveTable findings={findings}></CveTable>
}
...
function Check({ check }) {
    ...
    return (
        <div className={'container ' + styles.skanthisCallout + ' ml-4'}>
            ...
            <ul className="list-group list-group-flush ml-3 pt-3 border-top">
                <AdditionalFindings check={check}/>
            </ul>            
        </div>
