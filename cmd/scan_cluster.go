...
func NewCommandScanCluster() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			util.ConsolePrinter(fmt.Sprintf("Scan Completed within '%v'", color.HiBlueString(delta.String())))
			...
			if outfile != "-" {
				...
				if err != nil {
					...
					util.ConsolePrinter(fmt.Sprintf("Saving scanReport to '%v'", color.HiBlueString(outfile)))
					o = f
				}
			}
